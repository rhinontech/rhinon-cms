...
const CaseStudiesIndex = ({ caseStudies }: { caseStudies: CaseStudy[] }) => {
    ...
    return (
        <div className="relative flex flex-col overflow-hidden ">
            ...
            <div>
                <div className=" pt-44 max-md:pt-20 px-5 pb-10 flex flex-col gap-10 max-w-[1400px] mx-auto" >
                    ...
                    {caseStudies.length === 0 ? (
                        <p className="text-center text-white/60 py-20">Case studies coming soon.</p>
                    ) : (
                        <motion.div
                            className="grid grid-cols-2 max-md:grid-cols-1 gap-5"
                            initial={{ opacity: 0, y: 20, rotateX: 10, transformPerspective: 1000 }}
                            animate={{ opacity: 1, y: 0, rotateX: 0 }}
                            transition={{ duration: 0.8, ease: "easeOut", delay: 0.5 }}
                            viewport={{ once: true }}
                        >
                            {caseStudies.map((cs) => {
                                const cover = cs.image || cs.images?.[0] || "/images/background/q3lzdr4u88731.jpg";
                                return (
                                    <div
                                        key={cs.id}
                                        className="group border border-white/30 relative w-full h-[450px] max-md:h-[300px] overflow-hidden cursor-pointer"
                                        onClick={() => router.push(`/case-studies/${cs.slug}`)}
                                    >
                                        <img
                                            src={cover}
                                            alt={cs.title}
                                        ...
                                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center opacity-0 group-hover:opacity-100 transition-all duration-500">
                                            <h3 className="text-white text-center text-2xl font-[500]">{cs.title}</h3>
                                            <p className="text-gray-200 text-center text-sm tracking-wide">{cs.category || cs.industry}</p>
                                        </div>
                                    </div>
                                );
                            })}
