  ...
  if (loading) {
    return (
      <div className={cn("flex flex-col h-full glass-panel overflow-hidden", isSubNavExpanded ? "rounded-r-xl" : "rounded-xl")}>
        <header className="sticky top-0 z-10 flex items-center justify-between gap-4 h-16 px-5 border-b border-border glass-header">
                ...
                <div className="divide-y">
                  {cycles.filter((c) => c.status === "active").map((cycle) => (
                    <div key={cycle.id} className="px-5 py-3 flex items-center justify-between">
                      <div>
                        <p className="font-medium text-sm text-foreground">{cycle.name}</p>
                        <p className="text-xs text-muted-foreground mt-0.5">{cycle.startDate} — {cycle.endDate}</p>
                      </div>
                      <span className="text-xs rounded-md px-2 py-0.5 bg-green-50 dark:bg-green-400/10 text-green-700 dark:text-green-300 font-medium capitalize">{cycle.type}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-xl glass-card p-4">
                <p className="text-xs text-muted-foreground flex items-center gap-1"><TbTarget size={14} /> Goals Completed</p>
                <p className="mt-1 text-2xl font-semibold text-foreground">{completedGoals} / {goals.length}</p>
              </div>
              <div className="rounded-xl glass-card p-4">
                <p className="text-xs text-muted-foreground flex items-center gap-1"><TbStar size={14} /> Pending Reviews</p>
                <p className="mt-1 text-2xl font-semibold text-foreground">{pendingReviews}</p>
              </div>
            </div>

            {/* Pending self-assessment callout */}
            {pendingReviews > 0 && (
              <div className="rounded-xl border border-amber-200 dark:border-amber-400/25 bg-amber-50 dark:bg-amber-400/10 p-4">
                <p className="text-sm font-medium text-amber-800 dark:text-amber-200">Your self-assessment is due</p>
                ...
                <div className="divide-y">
                  {goals.slice(0, 5).map((goal) => (
                    <div key={goal.id} className="px-5 py-3 space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">{goal.progress}% complete{goal.targetDate ? ` · Due ${goal.targetDate}` : ""}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {goals.length === 0 && (
              <div className="rounded-xl glass-card p-8 text-center">
                <TbTarget size={32} className="mx-auto text-muted-foreground/70 mb-2" />
                <p className="text-sm text-muted-foreground">No goals yet. Add your first goal under My Goals.</p>
              </div>
            )}
          </>
        )}
