      ...
      {[1, 2, 3, 4, 5].map((n) => (
        ...
        >
          {(value ?? 0) >= n ? (
            ...
            <TbStar className="text-muted-foreground/70" />
          )}
        </button>
      ))}
        ...
        <div className="flex-1 overflow-auto p-5 space-y-6">
          {loading ? (
            ...
            <>
              {/* Self Assessments */}
              <section>
                <h3 className="text-sm font-semibold text-foreground/85 mb-3">My Self-Assessments</h3>
                  ...
                  <div className="space-y-2">
                    {selfReviews.map((sub) => (
                      <button
                        ...
                        className="w-full text-left rounded-xl glass-card p-4 hover:border-blue-200 dark:hover:border-blue-400/25 transition-colors"
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="font-medium text-sm text-foreground">{sub.cycle?.name ?? "—"}</p>
                            <p className="text-xs text-muted-foreground mt-0.5 capitalize">{sub.cycle?.type}</p>
                          </div>
                          <div className="flex items-center gap-3">
              ...
              {managerReviews.length > 0 && (
                <section>
                  <h3 className="text-sm font-semibold text-foreground/85 mb-3">Manager Reviews I've Given</h3>
                  <div className="space-y-2">
                    {managerReviews.map((sub) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="font-medium text-sm text-foreground">{sub.reviewee?.fullName ?? "—"}</p>
                            <p className="text-xs text-muted-foreground mt-0.5">{sub.cycle?.name} · {sub.reviewee?.department}</p>
                          </div>
                          <div className="flex items-center gap-3">
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                <div className="rounded-lg border border-border p-3 space-y-1">
                  ...
                  <p className="font-semibold text-foreground">{selected.cycle?.name ?? "—"}</p>
                </div>
                {selected.reviewee && (
                  <div className="rounded-lg border border-border p-3 space-y-1">
                    <p className="text-xs text-muted-foreground">Reviewee</p>
                    <p className="font-semibold text-foreground">{selected.reviewee.fullName}</p>
                    <p className="text-xs text-muted-foreground">{selected.reviewee.department}</p>
                  </div>
                )}
                ...
                <div className="space-y-1">
                  ...
                  {isReadOnly ? (
                    <div className="rounded-lg border border-border p-3 text-sm text-foreground/85 whitespace-pre-wrap">
                ...
                {isReadOnly && (
                  <div className="rounded-lg border border-green-100 dark:border-green-400/20 bg-green-50 dark:bg-green-400/10 p-3 text-sm text-green-700 dark:text-green-300 text-center font-medium">
                    Review submitted
                  </div>
                )}
              </div>
