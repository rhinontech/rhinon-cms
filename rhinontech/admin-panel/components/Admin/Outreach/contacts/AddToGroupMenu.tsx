  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button className="inline-flex items-center gap-2 rounded-lg px-3 py-1.5 text-xs font-medium text-blue-900 hover:bg-white">
          <TbUsersGroup size={14} /> Add to Group
        </button>
      </PopoverTrigger>
      <PopoverContent align="end" className="w-64 p-2">
        <p className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-stone-400">Existing groups</p>
        <div className="max-h-40 overflow-auto">
          {groups.length === 0 ? (
            <p className="px-2 py-2 text-xs text-stone-400">No groups yet.</p>
          ) : (
            groups.map((g) => (
              <button
                key={g.id}
                ...
                disabled={saving}
                className="flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-left text-sm hover:bg-stone-100 disabled:opacity-60"
              >
                <span className="truncate">{g.name}</span>
                <span className="text-xs text-stone-400">{g.memberCount}</span>
              </button>
            ))
          )}
        </div>
        <div className="mt-2 flex items-center gap-2 border-t pt-2">
          <input
            ...
            className="w-full rounded-lg border border-stone-200 px-2 py-1.5 text-xs outline-none focus:ring-2 focus:ring-blue-500"
          />
          <button
