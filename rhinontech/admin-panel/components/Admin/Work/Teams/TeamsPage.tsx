      ...
      <main className={cn("flex h-full min-h-0 w-full flex-col overflow-hidden glass-panel", isSubNavExpanded ? "rounded-r-xl" : "rounded-xl")}>
        <div className="flex h-16 items-center justify-between border-b px-4">
          <div className="flex items-center gap-3">
            <SubNavToggle />
            <div>
              <h1 className="text-base font-semibold tracking-tight text-gray-900">Teams</h1>
              <p className="text-xs text-gray-500">Private groups that can own projects only their members can see.</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-auto p-4">
          ...
          <div className="mt-8 overflow-hidden rounded-xl glass-card">
            <div className="min-w-[620px] grid grid-cols-[1.6fr_2fr_0.7fr_0.7fr] border-b bg-stone-100 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-gray-500">
              <span>Team</span>
              <span>Members</span>
              <span>People</span>
              <span>Projects</span>
            ...
            {visibleTeams.map((team) => (
              <button
                ...
                className={cn(
                  "grid w-full grid-cols-[1.6fr_2fr_0.7fr_0.7fr] items-center border-b px-4 py-3 text-left text-sm hover:bg-stone-50",
                  selected?.id === team.id && "bg-blue-50 hover:bg-blue-50"
                )}
              >
                <span className="flex items-center gap-2 font-medium text-gray-900">
                  <TbUsers size={16} className="shrink-0 text-gray-400" />
                  <span className="truncate">{team.name}</span>
                </span>
                <span className="truncate text-gray-600">
                  {team.members.map((m) => m.user?.fullName).filter(Boolean).join(", ") || "—"}
                </span>
                <span className="text-gray-600">{team.members.length}</span>
                <span className="text-gray-600">{team.projectCount ?? 0}</span>
              </button>
            ))}
            ...
            {loading && <div className="px-4 py-10 text-center text-sm text-gray-400">Loading teams…</div>}
          </div>
        </div>
      </main>

      <aside className={`min-h-0 flex-col bg-white overflow-hidden transition-all duration-200 ease-in-out ${mobileDetail ? "fixed inset-0 z-50 flex" : "hidden"} lg:static lg:z-auto lg:flex lg:h-full lg:rounded-xl ${isPreviewExpanded && (visibleTeams.length > 0 || mode === "create") ? "lg:w-[42%] lg:ml-2" : "lg:w-0"}`}>
        {isPreviewExpanded && (visibleTeams.length > 0 || mode === "create") && (
          <div className="flex h-full flex-1 flex-col overflow-hidden">
            <div className="sticky top-0 z-10 flex h-16 items-center justify-between border-b bg-white px-5">
              ...
              <div className="flex items-center gap-2">
                {mode === "view" && selected && (
                  <button onClick={deleteTeam} className="rounded-lg px-2 py-1.5 text-sm font-medium text-red-600 hover:bg-red-50">
                    <TbTrash size={16} />
                  </button>
                )}
            ...
            {mode === "create" ? (
              <form onSubmit={handleCreate} className="flex-1 space-y-4 overflow-auto p-5">
                ...
                <label className="flex flex-col gap-1 text-sm font-medium text-gray-700">
                  ...
                  <textarea
                    ...
                    className="min-h-24 rounded-lg border border-gray-200 px-3 py-2 font-normal outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </label>
                ...
                <div className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                  ...
                  <div className="max-h-64 overflow-auto rounded-lg border border-gray-200">
                    {people.map((person) => {
                      ...
                      return (
                        <label key={person.id} className="flex cursor-pointer items-center gap-3 border-b px-3 py-2 text-sm font-normal last:border-b-0 hover:bg-stone-50">
                          <input
                            type="checkbox"
                            checked={checked}
                            onChange={() => setForm((c) => ({
                            ...
                            className="h-4 w-4"
                          />
                          <span className="flex-1 truncate text-gray-800">{person.fullName}</span>
                          <span className="truncate text-xs text-gray-400">{person.department || ""}</span>
                        </label>
                      );
                    })}
                    {!people.length && (
                      <div className="px-3 py-6 text-center text-xs font-normal text-gray-400">
              ...
              <div className="flex-1 space-y-5 overflow-auto p-5">
                <div>
                  ...
                  <p className="mt-2 text-sm text-gray-500">{selected.description || "No description."}</p>
                </div>
                    ...
                    {selected.members.map((member) => (
                      <div key={member.id} className="flex items-center gap-2 border-b px-3 py-2.5 text-sm last:border-b-0">
                        <span className="flex-1 truncate text-gray-800">
                          ...
                          {member.role === "owner" && (
                            <TbCrown size={13} className="ml-1.5 inline text-amber-500" title="Team owner" />
                          )}
                        </span>
                        <select
                          value={member.role}
                          onChange={(event) => setMemberRole(member.userId, event.target.value as "owner" | "member")}
                          className="rounded-md border border-gray-200 bg-white px-2 py-1 text-xs outline-none focus:ring-2 focus:ring-blue-500"
                        >
              ...
              <div className="flex h-full items-center justify-center text-sm text-gray-400">Select a team.</div>
            )}
          </div>
        )}
      </aside>
