...
export function ProjectCollaborators({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="rounded-lg border border-gray-100 p-3">
      ...
      {open && (
        <form onSubmit={invite} className="mt-3 space-y-2 rounded-lg bg-stone-50 p-3">
          <input
            value={form.fullName}
          ...
          <button
            type="submit"
            disabled={saving}
            className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-stone-900 px-4 py-2 text-xs font-medium text-white hover:bg-stone-800 disabled:opacity-60"
          >
      ...
      <div className="mt-3 space-y-1.5">
        {rows.map((row) => (
          <div key={row.id} className="flex items-center gap-2 text-sm">
            <span className="min-w-0 flex-1 truncate text-gray-800">
              ...
              {row.access === "view" && (
                <span className="ml-1.5 text-[10px] text-gray-400">view only</span>
              )}
            </span>
            <button
        ...
        {!rows.length && !open && (
          <p className="text-xs text-gray-400">No external collaborators on this project.</p>
        )}
      </div>
    </div>
  );
