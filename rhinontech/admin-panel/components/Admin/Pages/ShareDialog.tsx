    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        {loading ? (
          ...
          <div className="py-8 text-center text-sm text-muted-foreground">Only the page owner can manage sharing.</div>
        ) : (
          <div className="space-y-4">
            ...
            >
              <span className="flex items-center gap-2 text-sm font-medium text-foreground">
                {visibility === "workspace" ? <TbWorld size={16} className="text-emerald-600 dark:text-emerald-300" /> : <TbLock size={16} className="text-muted-foreground" />}
                {visibility === "workspace" ? "Everyone at Rhinon can view" : "Only invited people"}
              </span>
              <span className="text-xs font-semibold text-muted-foreground">{visibility === "workspace" ? "Make private" : "Make workspace-visible"}</span>
            </button>
            ...
            <div className="flex gap-2">
              <select
                ...
                className="flex-1 rounded-lg border border-border bg-card px-2.5 py-2 text-sm outline-none focus:ring-2 focus:ring-ring"
              >
              ...
              <button
                onClick={addShare}
                disabled={!pickUserId}
                className="rounded-lg bg-primary px-3.5 py-2 text-sm font-medium text-primary-foreground disabled:opacity-40"
              >
                Add
              </button>
            </div>
            ...
            <div className="max-h-56 space-y-1.5 overflow-y-auto">
              {shares.length === 0 ? (
                <p className="py-3 text-center text-xs text-muted-foreground">Not shared with anyone individually yet.</p>
              ) : (
                shares.map((s) => (
                  <div key={s.userId} className="flex items-center justify-between rounded-lg border border-border px-3 py-2">
                    <span className="truncate text-sm text-foreground">{memberName(s.userId)}</span>
                    <div className="flex items-center gap-2">
                      <select
                        value={s.access}
                        onChange={(e) => setAccess(s.userId, e.target.value as "view" | "edit")}
                        className="rounded border border-border bg-card px-1.5 py-1 text-xs outline-none"
                      >
                        <option value="view">Can view</option>
                        <option value="edit">Can edit</option>
                      </select>
                      <button onClick={() => removeShare(s.userId)} className="text-muted-foreground/70 hover:text-red-600 dark:hover:text-red-300">
                        <TbTrash size={14} />
                      </button>
                    </div>
                  </div>
            ...
            <div className="flex justify-end gap-2 border-t border-border pt-3">
              <button onClick={() => onOpenChange(false)} className="rounded-lg px-3.5 py-2 text-sm font-medium text-foreground/70 hover:bg-muted">
                Cancel
              </button>
              <button onClick={save} disabled={saving} className="rounded-lg bg-primary px-3.5 py-2 text-sm font-medium text-primary-foreground disabled:opacity-50">
                {saving ? "Saving…" : "Save"}
              </button>
            </div>
          </div>
