...
function PasswordRule({ met, text }: { met: boolean; text: string }) {
  return (
    <span className={`flex items-center gap-1 text-xs ${met ? "text-green-600 dark:text-green-300" : "text-muted-foreground"}`}>
    ...
    <div className="min-h-screen bg-muted/40 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground">Rhinon Tech · Admin Panel</p>
        </div>
        ...
        {stage === "invalid" && (
          <div className="bg-card rounded-2xl border border-border p-10 text-center">
            ...
            <p className="text-sm text-muted-foreground">This onboarding link has expired or already been used. Contact your admin to resend the invite.</p>
          </div>
        )}
        ...
        {stage === "form" && (
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
            <div className="bg-primary px-6 py-5">
              <p className="text-primary-foreground font-semibold text-lg">Welcome, {firstName}!</p>
              <p className="text-primary-foreground/70 text-sm mt-0.5">Set your password to activate your account</p>
            </div>
            <div className="px-6 py-5">
              {/* Account info */}
              <div className="bg-muted/40 rounded-lg border border-border px-4 py-3 mb-5">
                <p className="text-xs text-muted-foreground mb-1">Your company email</p>
                <p className="text-sm font-semibold text-foreground">{companyEmail}</p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-4">
                {/* New password */}
                <div className="flex flex-col gap-1">
                  <label className="text-sm font-medium text-foreground/85">New Password</label>
                  <div className="flex items-center rounded-lg border border-border overflow-hidden focus-within:ring-2 focus-within:ring-blue-500">
                    <input
                      ...
                      required
                    />
                    <button type="button" onClick={() => setShowPw((v) => !v)} className="px-3 text-muted-foreground hover:text-foreground/70">
                ...
                {/* Confirm password */}
                <div className="flex flex-col gap-1">
                  <label className="text-sm font-medium text-foreground/85">Confirm Password</label>
                  <div className="flex items-center rounded-lg border border-border overflow-hidden focus-within:ring-2 focus-within:ring-blue-500">
                    <input
                      ...
                      required
                    />
                    <button type="button" onClick={() => setShowConfirm((v) => !v)} className="px-3 text-muted-foreground hover:text-foreground/70">
                ...
                <button
                  ...
                  className="w-full bg-primary text-primary-foreground rounded-lg py-2.5 text-sm font-semibold hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
        ...
        {stage === "success" && (
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
            ...
            <div className="px-6 py-5">
              <p className="text-sm text-foreground/70 mb-4">Your account is ready. Sign in with your company email to get started.</p>
              <div className="bg-muted/40 rounded-lg border border-border px-4 py-3 mb-5">
                <p className="text-xs text-muted-foreground mb-1">Login with</p>
                <p className="text-sm font-semibold text-foreground">{companyEmail}</p>
              </div>
              <button
